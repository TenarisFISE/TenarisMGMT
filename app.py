...
df1['Task'] = df['FSS1 assigned'] + ", OD: " + df['OD'] + ", RIG: " + df['RIG'] + ", Well: " + df['Well']
...
df2 = pd.DataFrame()
for j in range(df['FSS2 assigned'].count()):
    if df.loc[j, 'FSS2 assigned'] != 'na':
        df2.loc[j, 'Task'] = df.loc[j, 'FSS2 assigned'] + ", OD: " + df.loc[j, 'OD'] + ", RIG: " + df.loc[j, 'RIG'] + ", Well: " + df.loc[j, 'Well']
        df2.loc[j, 'Start'] = df.loc[j, 'Expected date']
        df2.loc[j, 'Finish'] = df.loc[j, 'Finish date']
        # df2['Complete'] = df['Customer']
        df2.loc[j, 'Description'] = df.loc[j, 'Customer']
        df2.loc[j, 'Resource'] = df.loc[j, 'Type']
frames = [df1, df2]
...
fig = ff.create_gantt(df1, group_tasks=True, colors=colors, index_col='Resource', reverse_colors=False, show_colorbar=True, height = 1000)
# </editor-fold>
...
# <editor-fold desc="Gantt Chart Analysis">
def calcMon():
...
fig_gca = ff.create_gantt(gantt_chart_analysis, height = 1000)
# </editor-fold>
...
layout_index = html.Div([
    ...
    dcc.Link(html.Button('Gantt Chart'), href='/page-1'),
    dcc.Link(html.Button('Utilization'), href='/page-2'),
    html.Br(),
...
# <editor-fold desc="Index Callbacks">
